...
const VendorDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (filterStatus === 'all') {
      ...
      setFilteredBookings(dateFilteredBookings.filter(b => b.status === 'booked' && !b.completed));
    } else if (filterStatus === 'completed') {
      setFilteredBookings(dateFilteredBookings.filter(b => b.status === 'done' || b.completed));
    } else {
      setFilteredBookings(dateFilteredBookings.filter(b => b.status === filterStatus));
    }
  ...
  const handleToggleShop = async () => {
    ...
    try {
      ...
      if (res.data && res.data.isOpen !== undefined) {
        ...
        showNotification(`Shop is now ${res.data.isOpen ? 'open' : 'closed'} for bookings`);
      } else {
  ...
  const handleStatusChange = async (bookingId, status, completedBy = '') => {
    ...
    try {
      ...
      await fetchBookings();
      showNotification(`Booking ${status === 'done' ? 'completed' : status === 'cancelled' ? 'cancelled' : 'updated'} successfully`);
  ...
  const handleTokenUpdate = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to update token number. Please try again.');
    } finally {
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        {notification.show && (
          <div className={`mb-4 p-3 ${notification.type === 'success' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'} border rounded transition-opacity duration-300`}>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between">
                  <div className="flex items-center mb-4 md:mb-0">
                    ...
                    <div className="ml-6 text-white">
                      <p className="text-xl md:text-2xl font-semibold">{currentToken.customerName || "Client"}</p>
                      <p className="text-lg"><span className="opacity-75">Phone:</span> {currentToken.customerPhone}</p>
                      <p className="text-lg"><span className="opacity-75">Service:</span> {currentToken.serviceName}</p>
                      <p className="text-lg"><span className="opacity-75">Time:</span> {currentToken.time}</p>
                      <p className="text-lg">
                        ...
                        {isEditingWaitTime ? (
                          <div className="inline-flex items-center">
                            ...
                            <button
                              ...
                              disabled={actionLoading[currentToken._id] === 'wait-time'}
                            >
                              ✓
                            </button>
                            <button
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    >
                      {actionLoading[currentToken._id] === 'notify' ? 'Sending...' : 'Notify Customer'}
                    </button>
        ...
        {!currentToken && !loading && (
          <div className="mb-8 bg-gradient-to-r from-gray-100 to-gray-200 border-l-4 border-gray-400 p-8 rounded-lg shadow-md text-center">
            ...
            <h2 className="text-2xl font-semibold text-gray-700 mb-3">No Active Tokens</h2>
            <p className="text-gray-600">There are no customers currently waiting to be served.</p>
          </div>
        )}

        // Improved shop status display section
{vendor && (
  <div className="mb-6 bg-white p-6 rounded-lg shadow-md border border-gray-200">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
      <div className="mb-4 sm:mb-0">
        ...
        <p className="text-gray-600">{vendor.location || 'No address provided'}</p>
      </div>
      
      <div className="flex flex-col items-center">
        <div className="flex items-center mb-3">
          <div className={`w-3 h-3 rounded-full mr-2 ${vendor.isOpen ? 'bg-green-500' : 'bg-red-500'}`}></div>
          <span className={`font-medium ${vendor.isOpen ? 'text-green-600' : 'text-red-600'}`}>
            {vendor.isOpen ? 'Open for Bookings' : 'Closed for Bookings'}
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center p-8 bg-gray-50 rounded-lg shadow-inner border border-gray-200">
            ...
            <p className="text-gray-500">No bookings found for the selected date and filter.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filteredBookings.map(booking => (
              <div key={booking._id} className={`p-4 border rounded-lg shadow-sm transition ${
                booking.status === 'booked' && !booking.completed ? 'bg-white' : 
                booking.status === 'done' ? 'bg-green-50 border-green-200' : 
                booking.status === 'cancelled' ? 'bg-red-50 border-red-200' : 'bg-gray-50'
              }`}>
                <div className="flex justify-between flex-wrap gap-4">
                  <div className="space-y-1">
                    <h3 className="font-medium text-gray-900">{booking.customerName || 'Anonymous Client'}</h3>
                    ...
                    <p><strong className="inline-block w-20">Date:</strong> {new Date(booking.date).toLocaleDateString()}</p>
                    ...
                    {booking.notes && (
                      <p><strong className="inline-block w-20">Notes:</strong> {booking.notes}</p>
                    )}
                    <p>
                  ...
                  <div className="flex flex-col gap-2">
                    {booking.status === 'booked' && (
                      <>
                        ...
                        >
                          {actionLoading[booking._id] === 'notify' ? 'Sending...' : 'Notify Customer'}
                        </button>
                    ...
                    {booking.status === 'done' && (
                      <button
                        ...
                        disabled={actionLoading[booking._id]}
                      >
                        {actionLoading[booking._id] === 'booked' ? 'Processing...' : 'Reopen Booking'}
                      </button>
                    )}
                  </div>
                </div>
        ...
        {reschedulingBooking && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-lg w-full max-w-md">
              <h3 className="text-xl font-semibold mb-2">Reschedule Appointment</h3>
              <p className="mb-4 text-gray-600">
                Rescheduling appointment for {reschedulingBooking.customerName || reschedulingBooking.customerPhone}
              </p>
              
              <div className="bg-blue-50 p-3 rounded-md mb-4 text-sm">
                <p><strong>Current Date:</strong> {reschedulingBooking.date}</p>
                <p><strong>Current Time:</strong> {reschedulingBooking.time}</p>
                <p><strong>Service:</strong> {reschedulingBooking.serviceName}</p>
              </div>
